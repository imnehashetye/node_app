pipeline{
    ...
    stages{
        ...
        stage('Deploy'){
            steps{
                sh 'ssh -t -i ~/session.pem -o StrictHostKeyChecking=no ubuntu@107.22.133.69 && cd /home/ubuntu/ && sudo touch test-file && docker pull 022536480424.dkr.ecr.us-east-1.amazonaws.com/node_app:latest'
                sh 'docker ps'
